...
static string GenerateMarkdown(string repoRoot)
{
    ...
    var entries = OpenTelemetryDeprecatedSemconvCatalog.Entries;
    var exactCount = Count(entries, entry => entry.MigrationKind is SemconvMigrationKind.ExactRename or SemconvMigrationKind.ExactValueRename);
    var manualCount = Count(entries, entry => entry.MigrationKind is SemconvMigrationKind.ContextSensitive or SemconvMigrationKind.ManualReview);
    var removedCount = Count(entries, entry => entry.MigrationKind == SemconvMigrationKind.RemovedNoReplacement);
    ...
    sb.AppendLine("activity.SetTag(\"http.method\", \"GET\");        // OTSC0030 when live [Obsolete] metadata is unavailable; use \"http.request.method\".");
    sb.AppendLine("activity.SetTag(\"error.message\", message);      // OTSC0031 because the replacement is domain-specific.");
    sb.AppendLine("tags.Add(\"http.method\", \"GET\");               // OTSC0031 for ambiguous dictionaries until the payload flow is proven.");
    sb.AppendLine("meter.CreateHistogram<long>(\"system.memory.shared\"); // OTSC0030; use \"system.memory.linux.shared\".");
    sb.AppendLine("```");

    sb.AppendLine();
    sb.AppendLine("## Supplemental Catalog Summary");
    sb.AppendLine();
    sb.AppendLine($"Supplemental entries: {entries.Length}. Exact replacements: {exactCount}. Manual/context-sensitive: {manualCount}. Removed/no-replacement: {removedCount}. Guidance-only: {guidanceCount}.");
    sb.AppendLine();
    sb.AppendLine("| Version | Domain | Total | Exact | Manual/context | Removed/no replacement |");
    sb.AppendLine("| -- | -- | --: | --: | --: | --: |");
        ...
        .ThenBy(group => group.Key.Domain, StringComparer.Ordinal))
    {
        ...
        sb.Append(" | ");
        sb.Append(Count(groupEntries, entry => entry.MigrationKind is SemconvMigrationKind.ExactRename or SemconvMigrationKind.ExactValueRename));
        sb.Append(" | ");
        sb.Append(Count(groupEntries, entry => entry.MigrationKind is SemconvMigrationKind.ContextSensitive or SemconvMigrationKind.ManualReview));
        sb.Append(" | ");
        sb.Append(Count(groupEntries, entry => entry.MigrationKind == SemconvMigrationKind.RemovedNoReplacement));
        sb.AppendLine(" |");
    }

    sb.AppendLine();
    sb.AppendLine("## Supplemental Catalog");
    sb.AppendLine();
...
static string VersionSortKey(string version) =>
    version == "unspecified" ? "0.0.0" : version;
