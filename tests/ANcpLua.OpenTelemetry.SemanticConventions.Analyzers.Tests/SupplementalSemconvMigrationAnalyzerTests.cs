...
public class SupplementalSemconvMigrationAnalyzerTests
{
            ...
            class C
            {
                void M(ILogger logger)
                {
                    logger.Log(
                        ...
                        new[]
                        {
                            new KeyValuePair<string, object?>({|#0:"event.name"|}, "legacy.event"),
                        });
                }
            }
            """;
    ...
    [Fact]
    public async Task ActivitySource_StartActivity_Local_Collection_Expression_Tags_Report_Production_Manual_Review_Once()
    {
        const string testCode = FakeTelemetry + """
    ...
    [Fact]
    public async Task Logger_BeginScope_State_Exact_Value_CodeFix_Replaces_Only_Value_Literal()
    {
        const string testCode = FakeTelemetry + """
    ...
    [Fact]
    public async Task Logger_BeginScope_Collection_Expression_Exact_Value_CodeFix_Replaces_Only_Value_Literal()
    {
        const string testCode = FakeTelemetry + """
