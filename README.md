...
- **Production payload recognition.** Supplemental diagnostics inspect visible inline payloads and local dictionary/collection initializers when they flow into telemetry emission APIs such as `ActivitySource.StartActivity` tags, metric instrument `Add`/`Record` calls, `Measurement<T>` tags, `ActivityEvent` tags, `ResourceBuilder.AddAttributes`, and `ILogger.Log`/`ILogger.BeginScope` state payloads.
