...
- **Production payload recognition.** Supplemental diagnostics inspect visible dictionary, collection-initializer, and `KeyValuePair<string, object?>` payloads when they flow into telemetry emission APIs such as `ActivitySource.StartActivity` tags, metric instrument `Add`/`Record` calls, `Measurement<T>` tags, `ActivityEvent` tags, `ResourceBuilder.AddAttributes`, and `ILogger.Log`/`ILogger.BeginScope` state payloads.
