...
- **Production payload recognition.** Supplemental diagnostics inspect visible dictionary, collection-initializer, and `KeyValuePair<string, object?>` payloads when they flow into telemetry emission APIs such as metric instrument `Add`/`Record` calls, `Measurement<T>` tags, `ActivityEvent` tags, and `ResourceBuilder.AddAttributes`.
